    ...
    stages {
        ...
        stage('Build') {
            steps {
                script {
                    sh """
                        sudo -E su ${projectUser} -c "docker compose -f ${dockerFile} build"
                    """
                }
            }
        }
        ...
        stage('Deploy') {
            steps {
                script {
                    sh """
                        sudo -E su ${projectUser} -c "docker compose -f ${dockerFile} down -v"
                        sudo -E su ${projectUser} -c "docker compose -f ${dockerFile} up -d"
                    """
                }
            }
        }
